...
max_AEI <-function(model, new.noise.var=0, y.min=NULL, type = "UK", lower, upper, parinit=NULL, control=NULL) {
	...
	AEI.envir <- new.env()
	environment(AEI) <- environment(AEI.grad_optim) <- AEI.envir 

	trend.names <- colnames(model@F)
	if (length(trend.names)==1) {
		if (trend.names=="(Intercept)") gr = AEI.grad_optim
	}
	else {
		gr = NULL
	}
...
#lower + runif(d) * (upper - lower)
	...
	o <- genoud(AEI, nvars=d, max=TRUE, 
		...
		cluster=FALSE, balance=FALSE, debug=FALSE, model=model, 
                new.noise.var=new.noise.var, y.min=y.min, type=type, envir=AEI.envir 
		)
